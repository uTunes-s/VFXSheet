...
function renderCameraTabs() {
  ...
  cameraListState.forEach((camera, index) => {
    ...
    tabButton.innerHTML = `<span>${getCameraReelName(camera)}</span><button type="button" onclick="editCameraReelName(${index}, event)" class="rounded border border-current/30 px-1 text-[10px] hover:bg-black/10">Edit</button>${cameraListState.length > 1 ? `<button type="button" onclick="removeCameraTab(${index}, event)" class="hover:text-rose-300 font-bold ml-1">✕</button>` : ''}`;
