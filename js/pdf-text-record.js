...
export async function renderTextPdfRecord(doc, record) {
  ...
  y = addSection('2. Multi-Camera Setup', [['Reel', 'Camera', 'Lens / Focal', 'T Stop / Shutter / FPS', 'ISO / WB / ND', 'Clip Name', 'Movement', 'Height / Distance / Tilt', 'LUT / Reference'], ...cameras.map(camera => [text(getCameraFieldLabel(camera)), text(camera.camera), `${text(camera.lens)} / ${text(camera.focal_length)}`, `${text(camera.t_stop)} / ${text(camera.shutter_speed)} / ${text(camera.frame_rate)}`, `${text(camera.iso_ei)} / ${text(camera.white_balance)} / ${text(camera.nd_filter)}`, text(camera.clip_name), text(camera.cramerawork), `${text(camera.height_value)} / ${text(camera.distance_value)} / ${text(camera.tilt_value)}`, `${text(camera.lut_info)} / Chart:${text(camera.flag_chart)} Clean:${text(camera.flag_cleanplate)} Ref:${text(camera.flag_reference)}`])], y);
